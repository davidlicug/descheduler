...
func GetPriorities(ctx context.Context, client clientset.Interface) ([]int32, error) {
	priorityClassList, err := client.SchedulingV1().PriorityClasses().List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
	...
	for _, priorityClass := range priorityClassList.Items {
		priority = append(priority, priorityClass.Value)
	}

	return priority, nil
}

func GetMinPriority(priority []int32)int32{
